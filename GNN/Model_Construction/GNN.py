...
"""
Define Classes for Graphical Neural Network
"""

class Trainer():
    def __init__(self, model, lrate, wdecay, clip, step_size, seq_out_len, device, cl=True, loss=0, comp='cpu'):
    ...
    def train(self, input, real_val, idx=None):
        ...
        output, A = self.model(input, idx=idx)
        output = output.transpose(1,3)
        real = torch.unsqueeze(real_val,dim=1)
        # predict = self.scaler.inverse_transform(output)
        predict = output
        # len = list(real.size())[3]
        # len = self.task_level
        if self.iter%self.step==0 and self.task_level<=self.seq_out_len:
            self.task_level +=1
        if self.cl:    
            loss = self.loss_fn(predict[:, :, :, :self.task_level], real[:, :, :, :self.task_level], 0.0)
        else:
        ...
        return loss.item(),rmse,mape, A
     
    def eval(self, input, real_val):
        self.model.eval()
        output, A = self.model(input) 
        output = output.transpose(1,3)
        real = torch.unsqueeze(real_val,dim=1)
        predict = output
        # predict = self.scaler.inverse_transform(output)
